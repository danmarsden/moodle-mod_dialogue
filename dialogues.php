...
/************** close conversation ************************************/
        ...
        add_to_log($course->id, 'dialogue', 'closed', "view.php?id=$cm->id");
        ...
        if ($conversations = dialogue_get_conversations($dialogue, $USER, array('closed = ?'), array(0))) {
            foreach ($conversations as $conversation) {
                ...
                if (isset($formdata->{$editorname}['text'])) {
                    ...
                    add_to_log($course->id, 'dialogue', 'add entry', "view.php?id=$cm->id");
        ...
        if (empty($params->recipientid)) {
            ...
            if ($recipients) {
                ...
                foreach ($recipients as $recipient) {
                    ...
                    add_to_log($course->id, 'dialogue', 'open', "view.php?id=$cm->id");
...
/****************** edit a reply ****************************************/
        ...
        if (isset($formdata->{$replyid})) {
            ...
            add_to_log($course->id, 'dialogue', 'edit entry', "view.php?id=$cm->id");
